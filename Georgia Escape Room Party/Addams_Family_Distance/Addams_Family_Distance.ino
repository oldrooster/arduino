...
const long interval = 100;  // interval to delay distance reader
unsigned long previousPulseMillis = 0;  // will store last time LED was updated
const long pulseSpeed = 4; //lower no for faster pulse of LED
...
void loop(){
  ...
  //pulse Eye 
  if (currentMillis - previousPulseMillis >= pulseSpeed) {
    // save the last time you blinked the LED
    previousPulseMillis = currentMillis;
    analogWrite(eyePin,i);
    if (i == maxLight)
    {
      reverse = true;
    }
